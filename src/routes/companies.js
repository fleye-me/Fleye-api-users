...
routes.post('/companies', (req, res) => {
  ...
  for (let u of user) {
    //caso usuario com esse id jah exista -> fazer uma funcao que retorna tds ids (util em mais lugares)
    ...
    if (u.id === user.id) {
      ...
      Object.entries(user).forEach(([key, value]) => {
        ...
        if (Company[key] !== '') {
          return res
            .status(400)
            .send(`The allowed information are: ${keys}`)
            .end();
...
routes.patch('/companies', (req, res) => {
